...
static inline seqlock_t wait_for_no_writer(const seqlock_t *sync, memory_order mo)
{
    seqlock_t l;
    SEVL(); /* Do SEVL early to avoid excessive loop alignment (NOPs) */
    if (UNLIKELY(((l = atomic_load_explicit(sync, mo)) & SEQLOCK_WRITER) != 0)) {
        while (WFE() && ((l = atomic_load_explicit(sync, mo)) & SEQLOCK_WRITER) != 0)
            spin_wait();
    }
...
void seqlock_acquire_wr(seqlock_t *sync)
{
    ...
    do {
        ...
        !atomic_compare_exchange_weak_explicit(sync, &l, l + SEQLOCK_WRITER,
...
void seqlock_release_wr(seqlock_t *sync)
{
    ...
    atomic_store_explicit(sync, cur + 1, memory_order_release);
}

#define ATOMIC_COPY(_d, _s, _sz, _type)                                      		\
    ({                                                                       		\
        _type val = atomic_load_explicit((const _type *) (_s), memory_order_relaxed);	\
        _s += sizeof(_type);                                                 		\
        atomic_store_explicit((_type *) (_d), val, memory_order_relaxed);            	\
        _d += sizeof(_type);                                                 		\
        _sz -= sizeof(_type);                                                		\
    })
